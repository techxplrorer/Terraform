...
resource "aws_subnet" "pubsub" {
  ...
  cidr_block              = element(var.cidr_public_subnet_web, count.index)
  availability_zone       = element(var.us_availability_zone, count.index)
  map_public_ip_on_launch = true
  tags = {
    Name = "Subnet-Public : Public Subnet Web Tier ${count.index + 1}"
  }
}

# Create Private Subnet within VPC for App Tier with Multi AZs 
resource "aws_subnet" "prisub_app" {
  count                   = length(var.cidr_private_subnet_app)
  vpc_id                  = aws_vpc.myVPC.id
  cidr_block              = element(var.cidr_private_subnet_app, count.index)
  availability_zone       = element(var.us_availability_zone, count.index)
  map_public_ip_on_launch = true
  tags = {
    Name = "Subnet-Private : Private Subnet App Tier ${count.index + 1}"
  }
}

# Create Private Subnet within VPC for DB Tier with Multi AZs 
resource "aws_subnet" "prisub_db" {
  count                   = length(var.cidr_private_subnet_db)
  vpc_id                  = aws_vpc.myVPC.id
  cidr_block              = element(var.cidr_private_subnet_db, count.index)
  availability_zone       = element(var.us_availability_zone, count.index)
  map_public_ip_on_launch = true
  tags = {
    Name = "Subnet-Private : Private Subnet DB Tier ${count.index + 1}"
  }
}
...
# Create Private Route Table for App private subnets 
resource "aws_route_table" "priRT_app" {
  count      = length(var.cidr_private_subnet_app)
  vpc_id     = aws_vpc.myVPC.id
  ...
  tags = {
    Name = "Private_RouteTable_App ${count.index + 1}"
  }
}

# Private Route Table Association for Private Subnets (App) with route table
resource "aws_route_table_association" "prirta_app" {
  count          = length(var.cidr_private_subnet_app)
  depends_on     = [aws_subnet.prisub_app, aws_route_table.priRT_app]
  subnet_id      = element(aws_subnet.prisub_app[*].id, count.index)
  route_table_id = aws_route_table.priRT_app[count.index].id
}

# Create Private Route Table for DB private subnets (NAT not required)
resource "aws_route_table" "priRT_db" {
  #count  = length(var.cidr_private_subnet_db)
  vpc_id = aws_vpc.myVPC.id
  tags = {
    Name = "Private_RouteTable_DB"
  }
}

# Private Route Table Association for Private Subnets (DB) with route table
resource "aws_route_table_association" "prirta_db" {
  count          = length(var.cidr_private_subnet_db)
  depends_on     = [aws_subnet.prisub_db, aws_route_table.priRT_db]
  subnet_id      = element(aws_subnet.prisub_db[*].id, count.index)
  route_table_id = aws_route_table.priRT_db.id
}
